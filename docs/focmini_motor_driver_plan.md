...
1. 暂时不要接 FOCMini 电机电源。
2. USB-TTL 改接 USART2：

   - STM32 PA2 -> USB-TTL RX
   - STM32 PA3 -> USB-TTL TX
...
5. 确认能看到启动日志：
   ...
   - `UART direct test`
   - `printf test`
   - `[OK ] ADC calibration done`
   - `AS5600 ADC/I2C compare start`
6. 确认 PC13 心跳灯继续闪烁。
7. 确认 AS5600 ADC/I2C 对比日志仍然持续打印。
8. 用万用表测 PB12 / FOCMINI_EN，确认默认低电平。
9. 如果有示波器或逻辑分析仪，测 PA8/PA9/PA10，确认默认 PWM 占空比为 0。
10. 只有以上检查都通过后，后续 PR 才考虑手动 enable 和低占空比开环测试。

## 9. 临时开环转动测试

这是临时硬件验证功能，不是正式 FOC 控制。

- 上电默认不会转。
- 串口发送 `t` 才启动 3 秒低占空比测试。
- 串口发送 `x` 可立即停止。
